...
const getLoadedMIDIFiles = () =>
  sequencer.getMidiFiles()
    .map((mf: Heartbeat.MIDIFileJSON) => mf.name);
...
const addAssetPacks = (aps: Array<Heartbeat.AssetPack>): Promise<void> => new Promise((resolve) => {
  let max = aps.length;
  aps.forEach(ap => {
    sequencer.addAssetPack(ap,
      () => {
...
const addAssetPack2 = (url: string): Promise<Heartbeat.AssetPack> => new Promise(async (resolve) => {
...
const createSongFromMIDIFile = (url: string): Promise<Heartbeat.Song> => new Promise(resolve => {
  sequencer.addMidiFile({ url }, (json: Heartbeat.MIDIFileJSON) => {
...
const parseConfig = async (config: Heartbeat.Config): Promise<ParseConfig> => new Promise(async (resolve) => {
  if (config.midiFiles) {
  ...
  if (config.assetPacks) {
    const aps: Array<Heartbeat.AssetPack> = await Promise.all(config.assetPacks.map(url => loadJSON(url)));
